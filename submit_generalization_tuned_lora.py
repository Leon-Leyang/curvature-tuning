...
def main(kwargs, job_dir):
    ...
    executor.update_parameters(
        ...
        slurm_job_name=f"gen_tuned_lora_rank{lora_rank}_{pretrained_ds}_to_{transfer_ds}_{model}_seed{seed}",  # Job name
    ...
    command = (
        ...
        f"python -u generalization_tuned_lora.py --model {model} --pretrained_ds {pretrained_ds} --transfer_ds {transfer_ds} --seed {seed} --transfer_test_bs {transfer_test_bs} --lora_rank {lora_rank}"
    )

    # Submit the job
    job = executor.submit(os.system, command)
    print(f"Job submitted for model {model} tuned with LoRA(rank {lora_rank}) on dataset {pretrained_ds}_to_{transfer_ds} with seed {seed} with job ID {job.job_id}")


def job_completed(pretrained_ds, transfer_ds, model, seed, lora_rank):
    transfer_ds_alias = transfer_ds.replace('/', '-')
    result_path = [
        f'./results/tuned_lora_rank{lora_rank}_{pretrained_ds}_to_{transfer_ds_alias}_{model}_seed{seed}.json',
    ]
