...
def main(kwargs, job_dir):
    ...
    executor.update_parameters(
        ...
        slurm_job_name=f"gen_train_ct_{pretrained_ds}_to_{transfer_ds}_{model}_seed{seed}",  # Job name
    ...
    command = (
        ...
        f"python -u generalization_trainable_ct.py --model {model} --pretrained_ds {pretrained_ds} --transfer_ds {transfer_ds} --seed {seed} --transfer_test_bs {transfer_test_bs}"
    )
...
def job_completed(pretrained_ds, transfer_ds, model, seed):
    ...
    result_path = [
        f'./results/train_ct_{pretrained_ds}_to_{transfer_ds_alias}_{model}_seed{seed}.json',
