...
def main(kwargs, job_dir):
    ...
    executor.update_parameters(
        ...
        slurm_job_name=f"search_ct_{pretrained_ds}_to_{transfer_ds}_{model}_seed{seed}",  # Job name
    ...
    command = (
        ...
        f"python -u classification_search_ct.py --model {model} --pretrained_ds {pretrained_ds} --transfer_ds {transfer_ds} --seed {seed} --linear_probe_test_bs {linear_probe_test_bs}"
    )
...
def job_completed(pretrained_ds, transfer_ds, model, seed):
    ...
    result_path = [
        f'./results/search_ct_{pretrained_ds}_to_{transfer_ds_alias}_{model}_seed{seed}.json',
    ]
